...
#保存到csv
    ...
    def save(self,list):
        cursor = self.conndb()
        sql = "INSERT INTO `commodities`( `CName`, `CLink`,`CLowestPrice`,`clowest_price`) values( %s,%s,%s,%s);"
        cursor.execute(sql,(list[0],list[3],list[1],list[1]))
...
if __name__ == '__main__':
    ...
    for i in range(1, len(sys.argv)):
        url = sys.argv[i]
        #url="http://item.jd.com/4824733.html"
        ul = []
        print(j.getname(url))
        ul.append(j.getname(url))#商品名称
        ul.append(j.getprice(url))#价格
        ul.append(j.getseller(url))#店铺
        ul.append(url)#链接
        print('https:' + url)
        print(j.getprice(url))
        print(j.getcomment(url))
        ul.extend(j.getcomment(url))#评论
        j.save(ul)